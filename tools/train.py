...
def parse_args():
    ...
    parser.add_argument('--suffix', help="Generic suffix to append to the folder name for uniqueness")
...
def main():
    ...
    if args.work_dir is not None:
        ...
        cfg.work_dir = osp.join('./work_dirs', cfg.get("group", ""), osp.splitext(osp.basename(args.config))[0])
    ...
    # add unique 'salt' and create work_dir
    if not args.suffix:
        suffix = f"{datetime.now().strftime('%Y-%m-%d')}_{random_string()}"
    else:
        suffix = args.suffix
    cfg.work_dir = f"{cfg.work_dir}_{suffix}"
