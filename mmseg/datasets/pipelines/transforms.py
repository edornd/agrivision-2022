...
class ResizeToMultiple(object):
    ...
    def __call__(self, results):
        ...
        img = results['img']
        img = mmcv.imresize_to_multiple(img,
                                        self.size_divisor,
                                        scale_factor=1,
    ...
    def __call__(self, results):
        ...
        Returns:
            dict: Normalized results, 'img_norm_cfg' key is added into
                result dict.
        """
        num_chs = results["img"].shape[-1]
        if self.to_rgb:
            img = results["img"]
            img[:, :, :3] = cv2.cvtColor(img[:, :, :3], cv2.COLOR_BGR2RGB)
            results["img"] = img
        normalized = mmcv.imnormalize(results['img'], self.mean[:num_chs], self.std[:num_chs], to_rgb=False)
        results['img'] = normalized
        results['img_norm_cfg'] = dict(mean=self.mean[:num_chs], std=self.std[:num_chs], to_rgb=self.to_rgb)
        return results
...
class CLAHE(object):
    ...
    def __call__(self, results):
        ...
        for i in range(results['img'].shape[2]):
            results['img'][:, :, i] = mmcv.clahe(np.array(results['img'][:, :, i], dtype=np.uint8), self.clip_limit,
                                                 self.tile_grid_size)
...
class RandomMosaic(object):
    ...
    def _mosaic_transform_img(self, results):
        ...
        if len(results['img'].shape) == 3:
            mosaic_img = np.full((int(self.img_scale[0] * 2), int(self.img_scale[1] * 2), 3),
                                 self.pad_val,
                                 dtype=results['img'].dtype)
        else:
            mosaic_img = np.full((int(self.img_scale[0] * 2), int(self.img_scale[1] * 2)),
                                 self.pad_val,
                                 dtype=results['img'].dtype)

        # mosaic center x, y
        self.center_x = int(random.uniform(*self.center_ratio_range) * self.img_scale[1])
        self.center_y = int(random.uniform(*self.center_ratio_range) * self.img_scale[0])
    ...
    def _mosaic_transform_seg(self, results):
        ...
        assert 'mix_results' in results
        for key in results.get('seg_fields', []):
            mosaic_seg = np.full((int(self.img_scale[0] * 2), int(self.img_scale[1] * 2)),
                                 self.seg_pad_val,
            ...
            for i, loc in enumerate(loc_strs):
                ...
                # keep_ratio resize
                scale_ratio_i = min(self.img_scale[0] / h_i, self.img_scale[1] / w_i)
                gt_seg_i = mmcv.imresize(gt_seg_i, (int(w_i * scale_ratio_i), int(h_i * scale_ratio_i)),
                                         interpolation='nearest')
